...
def use_setup_cfnsignaler_module_fixture(request):
    ...
    if any(marker.name == "use_setup_cfnsignaler_module" for marker in request.node.own_markers):
        ...
        logging.info(f"Marker for cfnSignaler found in module {request.node}")
        request.getfixturevalue('setup_environment_and_env_vars_for_cfnsignaler')
    else:
        logging.info('000000000000')


# autouse session fixture that will ensure aws mock is in place.
...
def create_eventbridge_rule():
    ...
    return _create_eventbridge_rule
...
def create_two_instances_sequentially():
    ...
    return {'ec2': ec2, 'instance1': instance1, 'instance2': instance2}
...
def setup_environment_and_env_vars_for_cfnsignaler(create_ssm_parameter, create_eventbridge_rule, create_two_instances_sequentially):
    ...
    os.environ['SchedulerSSMParameter'] = f"{os.environ['StackName']}/CFSignalerFunction/SchedulerFlag"

    create_ssm_parameter(name=os.environ['SchedulerSSMParameter'], value="enabled_increment_0")
    create_eventbridge_rule(name=os.environ['SchedulerName'], schedule_expression='cron(0 12 * * ? *)')

    # eventbridge_client = boto3.client('events')
    # response = eventbridge_client.describe_rule(Name='test-rule')

    # logging.info(boto3.client('ec2').describe_instances())
    # print('val ', create_two_instances_sequentially)
